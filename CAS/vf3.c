...
void f3_error_vector_rand(f3_vector *a, const int wt, prng_t *PRNG){
	uint8_t  xx, *x = (uint8_t*) malloc(a->size * sizeof(uint8_t));
	int r, j;
	int *p = (int*) malloc(a->size * sizeof(int));
	for (size_t j = 0; j < a->size; j++) {
		p[j]=j;
		if (j<wt){
			x[j]=f3_randnonzero(PRNG);
		}
		else{
			x[j]=0;
		}
	...
	randperm(p, a->size);
	for (j = 0; j < wt; j++) {
		r = j + rnd_short(a->size - j, PRNG); /* i <= r < n uniform */
		xx = x[r];
		x[r] = x[j];
		x[j] = xx;
	}
	...
	f3_vector_set_from_array(a, x, a->size);
	free(x);
	free(p);
}
...
if (f3_vector_get_coeff(x,j)!=f3_vector_get_coeff(y, j)){
	return 0;
}
}
return 1;
